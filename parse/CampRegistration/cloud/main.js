Parse.Cloud.afterSave("Camper", function(request) {
...
function sendEmail(to, name) {
  Parse.Config.get().then(function(config) {
    ...
    var sendgrid_password = config.get("sendgrid_password");

    var sendgrid = require("sendgrid");
    sendgrid.initialize("gsingh93", sendgrid_password);
    sendgrid.sendEmail({
        to: to,
        from: "camp_registration@michigangurudwara.com",
        subject: "Registration confirmation",
        text: "This email confirms that " + name + " has been successfully registered for this year's youth camp.\n\nPlease do not respond to this email."
...
Parse.Cloud.job("summaryEmail", function(request, status) {
    Parse.Config.get().then(function(config) {
        ...
        sendSummaryEmail(send_summary, owner_email, status, sendgrid_password);
    }, function(error) {
...
function sendSummaryEmail(send_summary, owner_email, status, sendgrid_password) {
    if (send_summary) {
...
function getNumRegistrations(callback) {
    var Student = Parse.Object.extend("Camp");
